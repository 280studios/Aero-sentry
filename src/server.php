<?php
//  server.php - Main Aero Sentry server

/* ===// Aero Sentry Server //===
1. Setting headers and starting session
2. Utility functions
3. Main request handler
  - Simple WAF included
4. Aero-Sentry handeling
  - Initialization & client info collection
  - PoW Challange, CSRF token generation
  - PoW verification and Bot mitigation
  - Checkbox form submission handling
5. Functions
  - CSRF token handling
  - Challenge handling
  - Bot detection
...
try {
  // [Optional] simple-WAF
  if (defined('ENABLE_WAF') && ENABLE_WAF && file_exists(__DIR__ . '/simple-waf.php')) {
    require_once __DIR__ . '/simple-waf.php';

    if (isRateLimited($_SERVER['REMOTE_ADDR'])) {
      jsonResponse(429, ["error" => "Rate limit exceeded. Please try again later."]);
    }
  }
...
function verifyClientMetadata() {
  ...
  // IP Test
    if (!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) {
      if(DEBUG) $_SESSION['client_debug']['ip'] = 'unknown'; // Debug
      $dataAnomaly += 2;
    }
  // User Agent Test
    if (isset($_SESSION['client_info']['userAgent'], $user_agent)) {
      if ($user_agent !== $_SESSION['client_info']['userAgent']) {
        if(DEBUG) $_SESSION['client_debug']['user_agent'] = 'notEqual: '.$user_agent;  // Debug
        $dataAnomaly += 1;
      }
      // User-agent string match from json file.
      if (isUserAgentBot($user_agent)) {
        if(DEBUG) $_SESSION['client_debug']['user_agent'] = 'botRegex: '.$user_agent;  // Debug
        $dataAnomaly += 1;
      }
    }
  // Language Test
    if (isset($_SESSION['client_info']['language'], $user_lang)) {
      if ($user_lang !== $_SESSION['client_info']['language']) {
        if(DEBUG) $_SESSION['client_debug']['language'] = 'notEqual: '.$user_lang; // Debug
        $dataAnomaly += 1;
      }
    } elseif (empty($user_lang)) {
      if(DEBUG) $_SESSION['client_debug']['language'] = 'empty'; // Debug
      $dataAnomaly += 1;
    }
  // Accept header Test

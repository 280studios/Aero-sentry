(function(){"use strict";const config={serverUrl:'./server.php',endPoint:'success.php',userAgentsFile:'data/user-agents.json',enableClientDataCollection:!0,showLoadingScreen:!0,anomaliesCountLimit:3,debug:!0,};let csrfToken='';let difficulty=3;let challenge='';let threshold=4;let userInteracted=!1;if(window.location.protocol!=='https:'){config.enableClientDataCollection=!1;console.warn('Warning: Server URL should be HTTPS! Data collection will be disabled.')}
...
return!0}
function redirectToSuccessPage(){if(document.querySelector('.spinner-small'))document.querySelector('.spinner-small').style.opacity='0';if(config.debug)log('Redirecting to success page...');if(config.debug)stopTimer();let url;url=new URL(config.endPoint,window.location.href);if(url&&url.origin===window.location.origin){window.location.replace(url);return}else{console.error('Invalid end-point. Please check your configuration.');return}}
let startTime=0;let timerRunning=!1;function startTimer(){if(!timerRunning){timerRunning=!0;startTime=new Date().getTime()}}
function stopTimer(){if(timerRunning){timerRunning=!1;const endTime=new Date().getTime();const timeTaken=endTime-startTime;if(config.debug)log(`Time taken: ${timeTaken}ms`);}}
async function initApp(){if(!checkNetworkStatus()){errorLog('No network connection.');return}
if(!verifyConfig())return;if(config.debug)startTimer();if(config.showLoadingScreen)loadLoadingScreen();try{const resultClient=await displayBotDetectionResults();if(resultClient){if(!config.debug)delete resultClient.detectionResult.anomalies;if(config.enableClientDataCollection&&window.location.protocol==='https:'){sendClientInfoGetCredentals(resultClient)}else{delete resultClient.browserComponents;sendClientInfoGetCredentals(resultClient)}
if(config.debug)log("Anomalies Count:",resultClient.detectionResult.anomaliesCount);if(config.debug)log("Browser Components:",resultClient.browserComponents);if(config.debug)log("Bot Detection Result:",resultClient.detectionResult);}else{sendClientInfoGetCredentals({collectDataFailed:!0})}}catch(error){errorLog("Error:",error)}}
...
if(config.enableClientDataCollection&&data.collectDataFailed){jsBotDetected=!0;if(config.debug)log('Data collection failed(while enabled), assuming bot.');}
data.nonice=generateNonce();if(config.debug)log('Sending client info...');const response=await fetch(config.serverUrl,{method:'POST',mode:'cors',cache:'no-cache',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'application/json','Referrer-Policy':'strict-origin'},body:JSON.stringify(data)});if(!response.ok){if(config.debug)throw new Error(`Server error: ${response.statusText}`);throw new Error('An error occurred while processing your request.')}
if(config.debug)log('Sending client info... Status code:',response.status);const result=await response.json();if(!result){throw new Error('Invalid server response')}
if(config.debug&&result.error)throw new Error(result.error.message);if(result.reload===!0){retryConnection(!1);return}
const date=new Date();const options={year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:!1};const timeString=date.toLocaleString(undefined,options);if(config.debug)log(`Current time: ${timeString}`);csrfToken=result.csrfToken||'';challenge=result.challenge||'';difficulty=Number(result.difficulty)||difficulty;threshold=Number(result.threshold)||threshold;fastInitiateConnection(jsBotDetected)}catch(error){errorLog('Request failed:',error);errorStopLoadingScreen()}}
...
const vgText=document.querySelector('.vg-text');if(vgText)vgText.textContent='Validating...';if(config.debug)log('Submitting solution...');hasUserInteracted();if(!challenge||typeof challenge!=='string'){throw new Error('Missing or invalid challenge')}
const nonce=await solvePoW(challenge);const submitResponse=await fetch(config.serverUrl,{method:'POST',mode:'cors',cache:'no-cache',credentials:'include',headers:{'Content-Type':'application/json','Authorization':`Bearer ${csrfToken}`,'Accept':'application/json','Referrer-Policy':'strict-origin'},body:JSON.stringify({nonce})});if(!submitResponse.ok){if(config.debug)throw new Error(`Server error: ${response.statusText}`);throw new Error('An error occurred while processing your request.')}
...
if(!csrfToken||typeof csrfToken!=='string'){retryConnection(!0);throw new Error('Token not found')}
const interacted=hasUserInteracted();const response=await fetch(config.serverUrl,{method:'POST',mode:'cors',credentials:'include',headers:{'Content-Type':'application/json','Authorization':`Bearer ${csrfToken}`,'Accept':'application/json','Referrer-Policy':'strict-origin'},body:JSON.stringify({checkboxClicked:!0,interacted:interacted})});if(!response.ok){if(config.debug)throw new Error(`Server error: ${response.statusText}`);throw new Error('An error occurred while processing your request.')}
...
const hasRunRetry=sessionStorage.getItem('hasRunRetry')?!0:!1;if(config.debug)console.log('retryConnection');if(hasRunRetry!=='true'){sessionStorage.setItem('hasRunRetry','true');location.reload(!0)}}
...
function hasUserInteracted(){if(userInteracted)return!0;const onInteraction=()=>{userInteracted=!0;document.removeEventListener('mousemove',onInteraction);document.removeEventListener('scroll',onInteraction);document.removeEventListener('keydown',onInteraction);document.removeEventListener('touchstart',onInteraction);if(config.debug)log('User interaction detected');};document.addEventListener('mousemove',onInteraction);document.addEventListener('scroll',onInteraction);document.addEventListener('keydown',onInteraction);document.addEventListener('touchstart',onInteraction);return userInteracted}
